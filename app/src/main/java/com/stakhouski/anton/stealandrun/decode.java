...
public class decode {
    private static final int LEVEL_SIZE = 0x200;
    private static final int COLUMN_SIZE = 16;
    private static final int POS_NUM = 16;
    private static final int COLUMNS_NUM = 26;
    ...
    private static final int EMPTY_2 = 5;
    private static final int BRICK_2 = 6;
    private static final int POLE = 7;
    private static final int PLAYER = 8;
    private static final int ENEMY = 9;
    ...
    int[][] cell_codes = new int[COLUMN_SIZE][POS_NUM];
    int[][] level = new int[COLUMN_SIZE][COLUMNS_NUM];
    boolean print_raw_level = False;
    ...
    void decode_column(final char[] buffer, int col_no) {
        int i;
    ...
    void print_level() {
        ...
        if (print_raw_level) {
            final int M_NOTHING = 0;
            final int M_BRICK = 1;
            final int M_CONCRETE = 2;
            final int M_LADDER = 3;
            final int M_GOLD = 4;
            final int M_PLAYER = 5;
            final int M_ENEMY = 6;
            final int M_POLE = 7;

            //System.out.println(COLUMNS_NUM + " " + COLUMN_SIZE);
...
/*for(j = 0; j < COLUMNS_NUM; j++)
    ...
    int main(int argv) {
        char[] buffer = new char[ COLUMN_SIZE];
        String filename;
        int level_no;
        int ch;
        int cnt;
        ...
        for (cnt = 0; cnt < COLUMNS_NUM; cnt++) {
            if ((rd = fin.re fread(buffer, 1, COLUMN_SIZE, fin)) != COLUMN_SIZE) {
                if (rd == 0)
                    System.out.println("cannot read level");
                else
                    System.out.println("error reading level %d: read only %lu bytes instead of");
